...
return function(params)
  ...
  function M:getColliders(params)
    ...
    self:setSubSequences(sequence)
  end  

  function M:setSubSequences(sequence)
    for i=self.go.numChildren,1, -1 do
      local ch = self.go[i]
      if ch and ch.setSequence then
        ch:setSequence(sequence)
      end
    end
  end

  function M:playSubObjects(sequence)
    for i=self.go.numChildren,1, -1 do
      local ch = self.go[i]
      if ch and ch.play then
        ch:play()
      end
    end
  end
    ...
    self:playSubObjects(sequence)
	end

  function M:pause()
    for i=self.go.numChildren,1, -1 do
      local ch = self.go[i]
      if ch and ch.pause then
        ch:pause()
      end
    end
  end
  ...
  function M:update(event)
    ...
    local sequence = self.go.currentSequence
    self:play(sequence)
